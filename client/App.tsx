...
type Tab = "overview" | "gateways" | "traces" | "team" | "billing";
...
function BillingPanel({ summary, owner, busy, onCheckout, onPortal, onRefresh }: { summary: BillingSummary | null; owner: boolean; busy: boolean; onCheckout: (plan: "pro" | "team") => Promise<void>; onPortal: () => Promise<void>; onRefresh: () => Promise<void> }) {
  ...
  return <section>
    <div className="metrics"><Metric label="Requests this month" value={summary.usage.used}/><Metric label="Monthly request limit" value={summary.usage.limit}/><Metric label="Active gateways" value={summary.resources.gateways}/><Metric label="Active keys" value={summary.resources.activeKeys}/><Metric label="Members" value={summary.resources.members}/><Metric label="Member limit" value={summary.entitlements.memberLimit}/></div>
    ...
    <div className="gateway-grid">
      <article className="gateway-card"><div className="gateway-title"><div><h3>Free</h3><p>$0 / month</p></div>{summary.billing.plan === "free" && <span className="badge good">Current</span>}</div><div className="gateway-meta"><span>1 gateway</span><span>2 active keys</span><span>1 member</span><span>10,000 requests / month</span></div></article>
      <article className="gateway-card"><div className="gateway-title"><div><h3>Pro</h3><p>$19 / month</p></div>{summary.billing.plan === "pro" && <span className="badge good">Current</span>}</div><div className="gateway-meta"><span>10 gateways</span><span>25 active keys</span><span>3 members</span><span>100,000 requests / month</span></div>{owner && !activePaid && <div className="card-actions"><button className="primary" disabled={busy || !summary.stripe.checkoutConfigured || !summary.stripe.webhookConfigured} onClick={() => onCheckout("pro")}>Upgrade to Pro</button></div>}</article>
      <article className="gateway-card"><div className="gateway-title"><div><h3>Team</h3><p>$49 / month</p></div>{summary.billing.plan === "team" && <span className="badge good">Current</span>}</div><div className="gateway-meta"><span>50 gateways</span><span>100 active keys</span><span>15 members</span><span>1,000,000 requests / month</span></div>{owner && !activePaid && <div className="card-actions"><button className="primary" disabled={busy || !summary.stripe.checkoutConfigured || !summary.stripe.webhookConfigured} onClick={() => onCheckout("team")}>Upgrade to Team</button></div>}</article>
    </div>

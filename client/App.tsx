...
function Dashboard({ bootstrap }: { bootstrap: Bootstrap }) {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {tab === "traces" && <section><div className="filters"><input placeholder="Search tool, method, request ID" value={traceQuery} onChange={(e) => setTraceQuery(e.target.value)} /><select value={traceDecision} onChange={(e) => setTraceDecision(e.target.value)}><option value="">All decisions</option><option value="allowed">Allowed</option><option value="policy_denied">Policy denied</option><option value="unauthorized">Unauthorized</option><option value="rate_limited">Rate limited</option><option value="quota_exceeded">Quota exceeded</option><option value="upstream_error">Upstream error</option></select><button onClick={() => loadTraces()}>Refresh</button></div><div className="panel"><TraceTable traces={traces} /></div></section>}
      ...
      {keyGateway && workspace && <KeyModal workspace={workspace} gatewayId={keyGateway} onClose={() => setKeyGateway(null)} onCreated={async (secret) => { const createdFor = keyGateway; setKeyGateway(null); setNewSecret(secret); if (createdFor) await Promise.all([loadKeys(createdFor), loadOverview(), loadBilling()]); }} />}
